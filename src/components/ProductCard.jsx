...
export default function ProductCard({ product }) {
  ...
  // Initialize liked state from wishlist (lazy initializer)
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer">
      <Link to={`/ProductDetail/${product.id}`}>
        <div className="relative overflow-hidden">
          ...
          {/* New Badge - You can add this conditionally if needed */}
      ...
      <div className="p-6">
        ...
        <p className="text-gray-600 text-sm mb-3 line-clamp-1">
          By {product.author} {/* Changed from artist to author */}
        </p>
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500">{product.place}</span> {/* Changed from dimensions to place */}
        </div>

        {/* Add to Cart Button */}
        <button
          onClick={handleAddToCart}
          className="w-full mt-3 bg-linear-to-r from-yellow-600 to-blue-800 text-white py-2 rounded-lg font-medium shadow-md hover:shadow-xl"
        >
          Add to Cart
        </button>
